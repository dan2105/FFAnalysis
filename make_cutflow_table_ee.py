...
INPUT_DIR = "output_histograms_electrons_loose"
HIST_NAME = "Cutflow_CutFlow_ee_loose_ttbar"
OUTPUT_TEX = "Plots/cutflow_ee_loose_table.tex"

SAMPLES = {
    "data.root":     ("Data",                    False),  # (label, is_mc)
    "ttHijing.root": (r"$t\bar{t}$ HIJING",      True),
    "zee.root":    (r"$Z \to e^{+}e^{-}$ HIJING",  True),
}
...
lines.append(r"  \caption{Cutflow for the $t\bar{t} \to e^{+}e^{-}$ selection (Medium WP).}")
lines.append(r"  \label{tab:cutflow_ee_loose}")
